<img src="https://github.com/BaseUQ/.github/blob/main/img/mRNArchitect_banner_transparent.png?raw=true" alt="mRNArchitect banner" height="200" style="display: block; margin-bottom: 30px;"/>

**mRNArchitect** is used to design mRNA medicines. The software optimises primary mRNA sequences for enhanced expression, stability and low reactogenicity. mRNArchitect designs have been experimentally validated in different applications, experiments and  cell-types and tissues, with an accompanying catalog of of validated mRNA elements (UTRs, polyA, SPs, etc).

Access the mRNArchitect web interface at [http://www.basefacility.org.au/software](http://www.basefacility.org.au/software).

The GitHub repository is private, if you would like a locally hosted version, please contact [basedesign@aibn.uq.edu.au](mailto:basedesign@aibn.uq.edu.au).

## Support and Documentation

For more information, please refer to the **Help** section within the mRNArchitect interface. If you encounter any issues or have support requests or feedback, contact us at [basedesign@aibn.uq.edu.au](mailto:basedesign@aibn.uq.edu.au). Please provide a clear description with enough detail to allow the issue to be reproduced.
